...
class OrderBloc extends Bloc<OrderEvent, OrderState> {
  ...
  FutureOr<void> _cancelOrder(OrderEvent event, Emitter<OrderState> orderStateEmitter){
    Order order = event.props.first as Order;
    orderStateEmitter(const OrderCancelling());
    takeOrderUseCase.cancelOrder(order);
    orderStateEmitter(OrderCancelled(order));
  }
  FutureOr<void> _completeOrder(OrderEvent event, Emitter<OrderState> orderStateEmitter){
    Order order = event.props.first as Order;
